...
void SpriteRenderer::UpdateBuffer(const GameContext& gameContext)
{
	...
	//		release the buffer (DONE)
	//		update the buffer size (if needed)   (DONE)
	...
	for (unsigned int i{}; i < m_Sprites.size(); ++i)
	{
		memcpy(mappedResource.pData, &m_Sprites[i], sizeof(SpriteVertex));
	}
