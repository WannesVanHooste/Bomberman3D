...
void SoftwareSkinningScene_3::Update()
{
	...
	for (unsigned int i{}; i < m_SkinnedVertices.size(); ++i) {
		...
		if (i < 24) {
			if (m_SkinnedVertices[i].BlendWeight1 == 100)
			{
				...
				auto newTransform = boneTransform1 + boneTransform2;
				DirectX::XMStoreFloat3(&m_SkinnedVertices[i].TransformedVertex.Position, DirectX::XMVector3TransformCoord(pos, newTransform));
				DirectX::XMStoreFloat3(&m_SkinnedVertices[i].TransformedVertex.Normal, DirectX::XMVector3TransformNormal(normal, newTransform));
			}
		}
		else {
			if (m_SkinnedVertices[i].BlendWeight2 == 100)
			{
...
void SoftwareSkinningScene_3::CreateMesh(float length) {
	...
	m_SkinnedVertices.push_back(SkinnedVertex(DirectX::XMFLOAT3(-offset.x + pos.x, offset.y + pos.y, -offset.z + pos.z), norm, col,100,0));
	m_SkinnedVertices.push_back(SkinnedVertex(DirectX::XMFLOAT3(offset.x + pos.x, offset.y + pos.y, -offset.z + pos.z), norm, col,50,50));
	m_SkinnedVertices.push_back(SkinnedVertex(DirectX::XMFLOAT3(offset.x + pos.x, -offset.y + pos.y, -offset.z + pos.z), norm, col,50,50));
	m_SkinnedVertices.push_back(SkinnedVertex(DirectX::XMFLOAT3(-offset.x + pos.x, -offset.y + pos.y, -offset.z + pos.z), norm, col,100,0));
	//BACK
	...
	norm = DirectX::XMFLOAT3(0, 0, -1);
	m_SkinnedVertices.push_back(SkinnedVertex(DirectX::XMFLOAT3(-offset.x + pos.x, offset.y + pos.y, -offset.z + pos.z), norm, col,50,50));
	m_SkinnedVertices.push_back(SkinnedVertex(DirectX::XMFLOAT3(offset.x + pos.x, offset.y + pos.y, -offset.z + pos.z), norm, col,0,100));
	m_SkinnedVertices.push_back(SkinnedVertex(DirectX::XMFLOAT3(offset.x + pos.x, -offset.y + pos.y, -offset.z + pos.z), norm, col,0,100));
	m_SkinnedVertices.push_back(SkinnedVertex(DirectX::XMFLOAT3(-offset.x + pos.x, -offset.y + pos.y, -offset.z + pos.z), norm, col,50,50));
	//BACK
	norm = DirectX::XMFLOAT3(0, 0, 1);
